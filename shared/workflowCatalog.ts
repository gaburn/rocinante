...
function phase(
  ...
  options: Pick<WorkflowPhaseDefinition, 'optional' | 'initiallySkipped' | 'skippedForClassifications'> = {},
...
export const WORKFLOW_CATALOG: Readonly<Record<WorkflowMode, WorkflowModeDefinition>> = {
  simple: {
    ...
    phases: [
      ...
      phase('pr', 'PR', true, { optional: true, initiallySkipped: true }),
    ],
  },
  full: {
    id: 'full',
...
export function parseBugFixClassification(value: unknown): BugFixClassification | null {
  if (typeof value !== 'string') return null;
  const normalized = value.trim().toLowerCase().replace(/[\s_]+/g, '-');
  return normalized === 'confirmed' || normalized === 'unverified-external'
    ? normalized
    : null;
}
